...
from .sqlite_postgres_migration import auto_bootstrap_postgres
...
logger = logging.getLogger("trm.runtime")


@asynccontextmanager
async def runtime_lifespan(app_instance):
    async with _original_lifespan(app_instance):
        # Explicit one-time import of the current bundled data into persistent Neon.
        migration = auto_bootstrap_postgres()
        if migration.get("status") in {"applied", "already_applied"}:
            logger.info("PostgreSQL data bootstrap: %s", migration)
