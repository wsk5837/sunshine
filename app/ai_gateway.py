    ...
    *,
    ...
    if context:
        prompt = (
            ...
            "最终回答面向业务用户：先给结论，再给关键数据和建议；使用简洁中文，不要解释后端、上下文、工具调用或数据权限实现。"
            "除非用户明确询问权限、集成或创建能力，不要主动提及当前角色、权限范围、MCP、写操作开关、工具绑定或能力边界。"
            "可使用标准Markdown：标题最多三级，列表优先；仅在有多个同类对象需比较时使用表格，表格必须包含表头和分隔行。"
            "不得输出原始JSON、令牌、Markdown源码说明或开发测试用语。\n\n"
            f"【交互入口】{source}\n"
            f"【用户问题】{question}\n\n"
            f"【系统事实数据】\n{context}"
        )
