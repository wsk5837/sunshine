...
async function api(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    const error = new Error(response.status === 403 ? '' : (data.message || `请求失败 ${response.status}`));
    error.data = data;
    error.status = response.status;
    if (response.status === 403) schedulePermissionRefresh();
    throw error;
  }
  return data;
}
...
function downloadFilename(disposition, fallback) {
  const extended = String(disposition || '').match(/filename\*=UTF-8''([^;]+)/i);
  if (extended) {
    try { return decodeURIComponent(extended[1]); } catch {}
  }
  const regular = String(disposition || '').match(/filename="?([^";]+)"?/i);
  return regular?.[1] || fallback;
}

async function downloadProtectedFile(path, fallbackFilename, trigger = null) {
  ...
  try {
    const response = await fetch(path, {
      headers: state.sessionToken ? { 'X-Session': state.sessionToken } : {}
    });
    if (!response.ok) {
      let data = {};
      ...
      if (response.status === 403) schedulePermissionRefresh();
      throw new Error(response.status === 403 ? '' : (data.message || `下载失败 ${response.status}`));
    }
    const blob = await response.blob();
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = downloadFilename(response.headers.get('Content-Disposition'), fallbackFilename || '下载文件');
    link.style.display = 'none';
    document.body.appendChild(link);
    link.click();
    link.remove();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
    toast('文件已开始下载', 'success');
  } catch (error) {
...
function schedulePermissionRefresh() {
  if (state.permissionRefreshPending || !state.sessionToken) return;
  state.permissionRefreshPending = true;
  setTimeout(async () => {
    try {
      ...
      state.currentUserData = me;
      state.actorId = me.username;
      state.userDisplay = `${me.display_name} ${me.username}`;
      state.role = me.role_code;
      closeModal();
      closeDrawer();
      updateUserUI();
      const base = parseRoute().path.split('/')[0];
      const target = canAccessRoute(base) ? base : defaultAccessibleRoute(base);
      if (!target) return;
      if (target !== base) window.history.replaceState(null, '', `#/${target}`);
      await renderRoute();
    } catch (error) {
      console.warn('权限状态刷新失败', error);
    } finally {
      state.permissionRefreshPending = false;
    }
  }, 0);
}
...
async function askAiAgent(question, { projectId = null, source = 'assistant' } = {}) {
  ...
  try {
    ...
    if ([401, 403].includes(remoteError.status)) throw remoteError;
...
function functionPointTotal(items = []) {
  return items.reduce((sum, fp) => sum + Number(fp.fp_count || 0), 0);
}

function amountCalculationTooltip(items = [], totalAmount = 0) {
...
function roleCanApprove(demand) {
  ...
  return hasPermission('demand.approve') && Boolean(roleByNode[demand?.current_node] && hasRole(roleByNode[demand.current_node]));
}
  ...
  try {
    ...
    if (draw) {
      ...
      $$('.notification-item').forEach((item) => item.addEventListener('click', async () => {
        ...
        closeDrawer();
        await loadNotifications(false);
        if (item.dataset.demand && canAccessRoute('demand-detail')) navigate(`demand-detail/${item.dataset.demand}`);
      }));
    }
  } catch (error) {
...
const ROUTE_ACCESS_RULES = {
  ...
  project360: { any: ['project360', 'project'] },
  value: { all: ['value'] },
  budget: { all: ['budget'] },
  ...
  'initiative-approvals': { all: ['initiative.approve', 'initiative.list'] },
  'demand-form': { all: ['demand.create'] },
  'demand-list': { all: ['demand.list'] },
  'demand-detail': { all: ['demand.list'] },
  approvals: { all: ['demand.approve', 'demand.list'] },
  approval: { all: ['demand.approve', 'demand.list'] },
  'product-eval': { all: ['demand.evaluate', 'demand.list'] },
  'function-points': { all: ['demand.list'], any: ['function_points', 'demand.evaluate'] },
  'function-point-new': { all: ['demand.list'], any: ['function_points', 'demand.evaluate'] },
  tapd: { all: ['tapd'] },
  ai: { all: ['ai'] },
  'project-list': { all: ['project'] },
  'project-detail': { all: ['project'] },
  'project-tasks': { all: ['project'] },
  milestones: { all: ['project'] },
  'project-governance': { all: ['project'] },
  'settlement-form': { all: ['settlement'] },
  'settlement-list': { all: ['settlement'] },
  'settlement-detail': { all: ['settlement'] },
  'settlement-approvals': { all: ['settlement'] },
  'indicator-list': { all: ['indicator'] },
  'indicator-data': { all: ['indicator'] },
  'indicator-board': { all: ['indicator'] },
  'contract-list': { all: ['contract'] },
  'contract-detail': { all: ['contract'] },
  'payment-plans': { all: ['contract'] },
  'contract-approvals': { all: ['contract'] },
  users: { all: ['system.users'] },
  roles: { all: ['system.roles'] },
  integrations: { all: ['system.integrations'] },
  audit: { all: ['system.audit'] }
};
...
function canAccessRoute(base) {
  const rule = ROUTE_ACCESS_RULES[base];
  if (!rule) return false;
  const allAllowed = (rule.all || []).every((permission) => hasPermission(permission));
  const anyAllowed = !rule.any?.length || rule.any.some((permission) => hasPermission(permission));
  return allAllowed && anyAllowed;
}

function defaultAccessibleRoute(exclude = '') {
  return ROUTE_FALLBACK_ORDER.find((route) => route !== exclude && canAccessRoute(route))
    || ROUTE_FALLBACK_ORDER.find((route) => canAccessRoute(route))
    || '';
}

function hasRole(role) {
  const roles = state.currentUserData?.role_codes || [state.role];
  return roles.includes('admin') || roles.includes(role);
}

function currentRoleCodes() {
  const roles = state.currentUserData?.role_codes;
  return Array.isArray(roles) && roles.length ? roles : [state.role].filter(Boolean);
}

function currentRoleLabels() {
  return (state.currentUserData?.role_labels || [state.currentUserData?.role_label || state.meta?.roles?.[state.role] || state.role]).join('、');
}

function applyMenuPermissions() {
  $$('[data-permission], [data-route]').forEach((item) => {
    const declaredAllowed = !item.dataset.permission || hasPermission(item.dataset.permission);
    const routeAllowed = !item.dataset.route || canAccessRoute(item.dataset.route);
    item.classList.toggle('permission-hidden', !declaredAllowed || !routeAllowed);
  });
...
function updateUserUI() {
  ...
  $('#userRoleLabel').textContent = currentRoleLabels();
  ...
  $('#menuRole').textContent = currentRoleLabels();
...
function bindShell() {
  document.addEventListener('click', (event) => {
    const trigger = event.target.closest('[data-protected-download]');
    if (!trigger) return;
    event.preventDefault();
    downloadProtectedFile(trigger.dataset.protectedDownload, trigger.dataset.filename || '下载文件', trigger);
  });
...
async function renderRoute() {
  ...
  try {
    ...
    if (error.status === 403) {
      try {
        state.currentUserData = (await api('/api/auth/me')).data;
        state.role = state.currentUserData.role_code;
        updateUserUI();
      } catch {}
      const fallback = defaultAccessibleRoute(base);
      heroActions.innerHTML = '';
...
async function renderDashboard() {
  const canApprove = canAccessRoute('approvals');
  ...
  const quickButtons = [
    ...
    hasPermission('project') ? btn('项目台账','btn','quickProject') : (hasPermission('project360') ? btn('项目360视图','btn','quickProject') : ''),
...
async function renderProject360() {
  ...
  if(!selected){appView.innerHTML='<div class="empty">暂无项目，请先发起立项；审批通过后将自动进入项目台账。</div>';return;}
  ...
  if ($('#p360Ask')) $('#p360Ask').addEventListener('click',askProject);
  if ($('#p360Q')) $('#p360Q').addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();askProject();}});
}
...
async function renderValueOverview() {
  ...
  const items=(await api('/api/business-values')).data;
  const canLinkProject=hasPermission('project360') || hasPermission('project');
  const projects=canLinkProject ? (await api('/api/projects')).data : [];
...
async function renderDemandForm() {
  ...
  if (idFromQuery && !hasPermission('demand.list')) return navigate('demand-form');
  ...
  const allowed = hasPermission('demand.create');
      ...
      <div>
        <div class="section">
          ...
          <div class="form-row"><div class="label">附件上传</div><div>
            ...
            ${demand?.attachments?.length ? `<div class="file-list" style="margin-top:8px">${demand.attachments.map((file) => `<div class="file-row"><span class="file-name">📎 ${esc(file.original_name)} <span class="status gray">${esc(file.category)}</span></span><span>${fmtSize(file.file_size)}</span><span class="action-group"><button class="link" type="button" data-protected-download="/api/attachments/${file.id}/download" data-filename="${esc(file.original_name)}">下载</button>${editable ? `<button class="link attachment-delete" data-id="${file.id}" type="button">删除</button>` : ''}</span></div>`).join('')}</div>` : ''}
          </div></div>
...
async function renderDemandDetail(id, query) {
  ...
  const tabs = [
    ['basic','基本信息'],
    ['approval','审批记录'],
    ...((hasPermission('function_points') || hasPermission('demand.evaluate')) ? [['fp','功能点评估']] : []),
    ...((hasPermission('budget') || hasPermission('demand.evaluate')) ? [['budget','预算与分摊']] : []),
    ...((hasPermission('tapd') || hasPermission('demand.evaluate') || hasPermission('project')) ? [['tapd','TAPD与工时']] : [])
  ];
...
function renderDemandDetailTab(demand, tab) {
  ...
  if (tab === 'tapd') {
    ...
    const deviationMessage = deviation > 30
      ? (warningDelivered
        ? '已向产品经理和项目经理发送工时偏差预警（实际超支口径）。'
        : '已超过30%预警阈值，通知正在补发，请刷新后查看。')
      : '当前未超过30%预警阈值。';
    const canManageWork = hasPermission('demand.evaluate') || hasPermission('project');
    const canApproveWork = currentRoleCodes().some((role) => ['admin','product_manager','project_manager'].includes(role));
    const dueDate = demand.expected_completion_date || demand.planned_online_date || '';
    const isClosed = ['已完成','已终止','已验收','已关闭','已拒绝'].includes(demand.status);
    const overdue = Boolean(dueDate && !isClosed && new Date(`${dueDate}T23:59:59`).getTime() < Date.now());
    const workRows = (demand.work_logs||[]).map(x=>{
      const approvalStatus=x.approval_status||'已通过';
      const actions=[];
      if (canApproveWork && approvalStatus==='待审批') {
        actions.push(`<button class="link work-log-approve" data-id="${x.id}">通过</button>`);
        actions.push(`<button class="link danger work-log-reject" data-id="${x.id}">驳回</button>`);
      }
      if (canManageWork && approvalStatus!=='已通过') actions.push(`<button class="link work-log-delete" data-id="${x.id}">删除</button>`);
      return [
        esc(x.work_date), esc(x.worker), `<span class="mono">${esc(x.fp_no||'—')}</span><div class="sub">${esc(x.function_point_name||'—')}</div>`,
        esc(x.task_name||'—'), `${Number(x.hours||0).toFixed(2)}h`, statusPill(approvalStatus), esc(x.description||'—'), actions.join(' ')||'—'
      ];
    });
  ...
  const functionPointSummary = (hasPermission('function_points') || hasPermission('demand.evaluate'))
    ...
    : '';
  return `<div class="info-grid interactive-info-grid"><div>需求编号</div><div>${esc(demand.demand_no || '提交后生成')}</div><div>需求类型</div><div>${esc(demand.demand_type)}</div><div>需求标题</div><div>${esc(demand.title)}</div><div>申请人</div><div>${esc(demand.applicant)}</div><div>优先级</div><div>${esc(demand.priority)}</div><div>当前节点</div><div>${esc(demand.current_node)}</div><div>预算出处</div><div>${(demand.budget_sources || []).map((x)=>`<span class="tag">${esc(x)}</span>`).join(' ') || '—'}</div><div>申请预算金额</div><div>¥ ${money(demand.budget_amount)}</div>${functionPointSummary}<div>需求描述</div><div>${esc(demand.description || '—')}</div></div><div class="section-title" style="margin-top:18px">附件</div>${demand.attachments.length ? `<div class="file-list">${demand.attachments.map((file)=>`<div class="file-row"><span class="file-name">📎 ${esc(file.original_name)} <span class="status gray">${esc(file.category)}</span></span><span>${fmtSize(file.file_size)}</span><button class="link" type="button" data-protected-download="/api/attachments/${file.id}/download" data-filename="${esc(file.original_name)}">下载</button></div>`).join('')}</div>` : '<div class="empty" style="min-height:80px">暂无附件</div>'}`;
}
...
function bindDetailTabActions(demand, tab) {
  ...
  if (tab === 'tapd') {
    ...
    $$('.work-log-approve, .work-log-reject').forEach((button) => button.addEventListener('click', async () => {
      const action=button.classList.contains('work-log-approve')?'通过':'驳回';
      const comment=action==='驳回' ? (prompt('请填写驳回原因')||'').trim() : '';
      if (action==='驳回' && !comment) return;
      try { const r=await api(`/api/work-logs/${button.dataset.id}/approve`,{method:'POST',body:JSON.stringify({action,comment})}); toast(r.message,'success'); await loadNotifications(false); renderRoute(); }
      catch(error){toast(error.message,'error');}
    }));
    $$('.work-log-delete').forEach((button) => button.addEventListener('click', async () => {
      if (!confirm('确认删除该工时记录？删除后实际工时将自动重新汇总。')) return;
      try { await api(`/api/work-logs/${button.dataset.id}`, {method:'DELETE'}); toast('工时记录已删除','success'); await loadNotifications(false); renderRoute(); }
      catch (error) { toast(error.message,'error'); }
    }));
  }
}

function openWorkPlanForm(demand) {
  showModal(`<h3 id="modalTitle">调整工时计划</h3><div class="grid-2"><div><label>预估工时 *</label><input id="workEstimated" type="number" min="0.01" step="0.25" class="field" value="${Number(demand.estimated_hours||0)}"></div><div><label>计划完成日期</label><input id="workDueDate" type="date" class="field" value="${esc(demand.expected_completion_date||demand.planned_online_date||'')}"></div></div><div style="margin-top:10px"><label>调整说明</label><textarea id="workPlanNote" class="textarea" maxlength="500" placeholder="记录评估依据或计划变更原因"></textarea></div><div class="modal-actions">${btn('取消','btn','workPlanCancel')}${btn('保存计划','btn primary','workPlanSave')}</div>`);
  $('#workPlanCancel').addEventListener('click', closeModal);
  $('#workPlanSave').addEventListener('click', async () => {
    const estimated_hours=Number($('#workEstimated').value);
    if (!(estimated_hours>0)) return toast('预估工时必须大于0','error');
    try { const r=await api(`/api/demands/${demand.id}/work-plan`,{method:'PUT',body:JSON.stringify({estimated_hours,expected_completion_date:$('#workDueDate').value||null,note:$('#workPlanNote').value})}); closeModal(); toast(r.message,'success'); await loadNotifications(false); renderRoute(); }
    catch(error){toast(error.message,'error');}
  });
}

function openWorkLogForm(demand) {
  const user=currentUser();
  const externalSource=String(demand.actual_hours_source||demand.work_hour_source||'').startsWith('TAPD');
  const points=demand.function_points||[];
  const fpOptions=points.map((fp)=>`<option value="${fp.id}">${esc(fp.fp_no)} · ${esc(fp.name)}· ${esc(fp.system_name)}</option>`).join('');
  showModal(`<h3 id="modalTitle">登记实际工时</h3>${!points.length?'<div class="callout warn">当前需求还没有功能点，请先完成功能点评估。</div>':''}${externalSource?'<div class="callout warn">当前实际工时来自TAPD。切换为人工维护后，已审批的人工工时将作为新口径。</div>':''}<div style="margin-top:10px"><label>关联功能点 *</label><select id="workLogFunctionPoint" class="field" ${points.length?'':'disabled'}><option value="">请选择</option>${fpOptions}</select></div><div class="grid-2" style="margin-top:10px"><div><label>工时日期 *</label><input id="workLogDate" type="date" class="field" max="${new Date().toISOString().slice(0,10)}" value="${new Date().toISOString().slice(0,10)}"></div><div><label>工时（小时）*</label><input id="workLogHours" type="number" min="0.01" max="24" step="0.25" class="field" value="8"></div></div><div class="grid-2" style="margin-top:10px"><div><label>登记人 *</label><input id="workLogWorker" class="field" maxlength="100" value="${esc(user.name)}"></div><div><label>工作事项</label><input id="workLogTask" class="field" maxlength="200" placeholder="例如：接口开发"></div></div><div style="margin-top:10px"><label>工作说明</label><textarea id="workLogDesc" class="textarea" maxlength="500" placeholder="填写完成内容"></textarea></div>${externalSource?'<label class="permission-all" style="margin-top:10px"><input id="replaceExternalHours" type="checkbox"> 确认改为人工工时口径</label>':''}<div class="modal-actions">${btn('取消','btn','workLogCancel')}${btn('提交审批','btn primary','workLogSave')}</div>`);
  $('#workLogCancel').addEventListener('click', closeModal);
  $('#workLogSave').addEventListener('click', async () => {
    const hours=Number($('#workLogHours').value),worker=$('#workLogWorker').value.trim(),function_point_id=Number($('#workLogFunctionPoint').value);
    if (!function_point_id) return toast('请选择关联功能点','error');
    if (!(hours>0&&hours<=24)) return toast('单笔工时必须大于0且不超过24小时','error');
    if (!worker) return toast('请填写登记人','error');
    if (externalSource && !$('#replaceExternalHours')?.checked) return toast('请先确认改为人工工时口径','error');
    try { const r=await api(`/api/demands/${demand.id}/work-logs`,{method:'POST',body:JSON.stringify({function_point_id,work_date:$('#workLogDate').value,hours,worker,task_name:$('#workLogTask').value,description:$('#workLogDesc').value,replace_external:Boolean($('#replaceExternalHours')?.checked)})}); closeModal(); toast(r.message,'success'); await loadNotifications(false); renderRoute(); }
    catch(error){toast(error.message,'error');}
  });
}
...
async function renderApprovalDetail(id) {
  ...
  if (demand.current_node === '产品经理审批') {
    const evaluationEntry = canAccessRoute('product-eval') ? `<div style="margin-top:8px">${btn('进入费用评估与预算','btn secondary','goProductEval')}</div>` : '';
    special = `<div class="callout ${demand.function_points.length ? 'success' : 'warn'}">产品经理节点必须完成至少1条功能点评估。当前已关联 ${demand.function_points.length} 条，预估金额 ¥${money(demand.estimated_amount)}。${evaluationEntry}</div>`;
  }
...
async function renderProductEval(query) {
  ...
  const editable = demand && (hasPermission('function_points') || hasPermission('demand.evaluate'));
...
function bindProductEval(demand, editable) {
  ...
  $('#fpTemplate')?.addEventListener('click', (event) => downloadProtectedFile('/api/function-points/template','function_point_template.xlsx',event.currentTarget));
  $('#fpExport')?.addEventListener('click', (event) => downloadProtectedFile(`/api/demands/${demand.id}/function-points/export`,`function_points_${demand.id}.xlsx`,event.currentTarget));
...
function openFunctionPointDetail(demand, fpId) {
  ...
  showModal(`<h3 id="modalTitle">功能点评估详情</h3><div class="info-grid interactive-info-grid"><div>功能点编号</div><div>${esc(fp.fp_no)}</div><div>来源</div><div>${esc(fp.source_type||'新增')}</div><div>归属系统</div><div>${esc(fp.system_name)}</div><div>需求名称</div><div>${esc(fp.name)}</div><div>需求概述</div><div>${esc(fp.demand_summary)}</div><div>评估人</div><div>${esc(fp.evaluator)}</div><div>所属部门</div><div>${esc(fp.department)}</div><div>所属团队</div><div>${esc(fp.team)}</div><div>评估日期</div><div>${esc(fp.evaluation_date)}</div><div>预估功能点</div><div>${Number(fp.fp_count).toFixed(2)}</div><div>单价</div><div>¥ ${money(fp.unit_price)}</div><div>预估金额</div><div>${amountCalculationTooltip([fp], fp.estimated_amount)}</div></div><div class="modal-actions">${btn('关闭','btn','closeFpDetail')}</div>`);
...
async function renderFunctionPointCatalog(query) {
  ...
  $('#catalogTemplate').addEventListener('click', (event) => downloadProtectedFile('/api/function-points/template','function_point_template.xlsx',event.currentTarget));
...
async function renderBudgetAllocation() {
  setPage({ title: '费用分摊台账', iconName: 'budget', crumbs: ['预算管理'], actions: btn('刷新','btn','allocationRefresh') });
  const items=(await api('/api/budget-allocations')).data;
  const occupied=items.filter(x=>x.ledger_status==='已占用').reduce((s,x)=>s+Number(x.amount||0),0);
  const pending=items.filter(x=>x.ledger_status==='待占用').reduce((s,x)=>s+Number(x.amount||0),0);
  const released=items.filter(x=>x.ledger_status==='已释放').reduce((s,x)=>s+Number(x.amount||0),0);
  appView.innerHTML=`<div class="grid-4"><div class="metric"><div class="k">分摊记录</div><div class="v">${items.length}</div></div><div class="metric"><div class="k">已占用</div><div class="v" style="font-size:20px">¥${money(occupied)}</div></div><div class="metric"><div class="k">待财务占用</div><div class="v" style="font-size:20px">¥${money(pending)}</div></div><div class="metric"><div class="k">已释放</div><div class="v" style="font-size:20px">¥${money(released)}</div></div></div>
  <div class="section" style="margin-top:12px"><div class="toolbar"><div><div class="section-title">需求费用分摊</div><div class="section-subtitle">财务审批通过时自动占用；后续审批驳回时自动释放。</div></div><input id="allocationSearch" class="field" style="max-width:320px" placeholder="搜索需求/功能点/预算"></div><div id="allocationLedgerRows"></div></div>`;
  const draw=()=>{const q=$('#allocationSearch').value.trim().toLowerCase();const filtered=items.filter(x=>!q||`${x.demand_no} ${x.demand_title} ${x.fp_no} ${x.function_point_name} ${x.budget_no} ${x.budget_name}`.toLowerCase().includes(q));$('#allocationLedgerRows').innerHTML=simpleTable(['需求','功能点','归属系统','预算','费用主体','比例','金额','归属部门','台账状态'],filtered.map(x=>[`<button class="link allocation-demand" data-id="${x.demand_id}">${esc(x.demand_no||'草稿')}</button><div class="sub">${esc(x.demand_title)}</div>`,`${esc(x.fp_no||'—')}<div class="sub">${esc(x.function_point_name||'—')}</div>`,esc(x.system_name||'—'),`${esc(x.budget_no||'—')}<div class="sub">${esc(x.budget_name||x.expense_source)}</div>`,esc(x.expense_subject),`${Number(x.ratio||0).toFixed(2)}%`,`¥${money(x.amount)}`,esc(x.department),statusPill(x.ledger_status||'待占用')]));$$('.allocation-demand').forEach(b=>b.addEventListener('click',()=>navigate(`demand-detail/${b.dataset.id}`,{tab:'budget'})));};
  $('#allocationSearch').addEventListener('input',draw);$('#allocationRefresh').addEventListener('click',renderBudgetAllocation);draw();
}

async function renderBudgetManpower() {
  setPage({ title: '项目概览（人力）', iconName: 'budget', crumbs: ['预算管理'] });
  const items=(await api('/api/budget-project-overview')).data;
  const estimated=items.reduce((s,x)=>s+Number(x.estimated_hours||0),0),approved=items.reduce((s,x)=>s+Number(x.approved_hours||0),0),pending=items.reduce((s,x)=>s+Number(x.pending_hours||0),0);
  appView.innerHTML=`<div class="grid-4"><div class="metric"><div class="k">项目数</div><div class="v">${items.length}</div></div><div class="metric"><div class="k">需求预估工时</div><div class="v">${estimated.toFixed(1)}h</div></div><div class="metric"><div class="k">已审批实际工时</div><div class="v">${approved.toFixed(1)}h</div></div><div class="metric"><div class="k">待审批工时</div><div class="v">${pending.toFixed(1)}h</div></div></div><div class="section" style="margin-top:12px"><div class="section-title">项目人力投入</div>${simpleTable(['项目','部门','需求数','预估工时','已审批工时','待审批','工时执行率','进度'],items.map(x=>[`<button class="link manpower-project" data-id="${x.id}">${esc(x.project_no)} · ${esc(x.name)}</button>`,esc(x.department),x.demand_count,`${Number(x.estimated_hours||0).toFixed(1)}h`,`${Number(x.approved_hours||0).toFixed(1)}h`,`${Number(x.pending_hours||0).toFixed(1)}h`,progressCell(x.estimated_hours?Math.min(100,x.approved_hours/x.estimated_hours*100):0),progressCell(x.progress)]))}</div>`;
  $$('.manpower-project').forEach(b=>b.addEventListener('click',()=>navigate(`project-detail/${b.dataset.id}`)));
}

async function renderBudgetFinance() {
  setPage({ title: '项目概览（财务）', iconName: 'budget', crumbs: ['预算管理'] });
  const items=(await api('/api/budget-project-overview')).data;
  const total=items.reduce((s,x)=>s+Number(x.budget_total||0),0),used=items.reduce((s,x)=>s+Number(x.budget_used||0),0),contract=items.reduce((s,x)=>s+Number(x.contract_amount||0),0),settled=items.reduce((s,x)=>s+Number(x.settlement_amount||0),0);
  appView.innerHTML=`<div class="grid-4"><div class="metric"><div class="k">关联预算</div><div class="v" style="font-size:20px">¥${money(total)}</div></div><div class="metric"><div class="k">已占用/支出</div><div class="v" style="font-size:20px">¥${money(used)}</div></div><div class="metric"><div class="k">合同金额</div><div class="v" style="font-size:20px">¥${money(contract)}</div></div><div class="metric"><div class="k">结算金额</div><div class="v" style="font-size:20px">¥${money(settled)}</div></div></div><div class="section" style="margin-top:12px"><div class="section-title">项目财务概览</div>${simpleTable(['项目','关联预算','总预算','已使用','剩余','执行率','合同','结算','预警'],items.map(x=>[`<button class="link finance-project" data-id="${x.id}">${esc(x.project_no)} · ${esc(x.name)}</button>`,`${esc(x.budget_no||'—')}<div class="sub">${esc(x.budget_name||'未关联')}</div>`,`¥${money(x.budget_total)}`,`¥${money(x.budget_used)}`,`¥${money(x.budget_remaining)}`,progressCell(Math.min(100,x.execution_rate)),`${x.contract_count}份 / ¥${money(x.contract_amount)}`,`${x.settlement_count}笔 / ¥${money(x.settlement_amount)}`,statusPill(x.execution_rate>=95?'预警':'正常')]))}</div>`;
  $$('.finance-project').forEach(b=>b.addEventListener('click',()=>navigate(`project-detail/${b.dataset.id}`)));
}

async function renderTapd() {
  const canConfigure = hasPermission('system.integrations');
  const canViewDemands = hasPermission('demand.list');
  setPage({ title: 'TAPD同步中心', iconName: 'sync', crumbs: ['集成中心'], actions: `${canConfigure?btn('执行后台任务','btn','tapdRunJobs'):''}${btn('刷新','btn','tapdRefresh')}` });
  const [demandResp, settingResp, overviewResp] = await Promise.all([
    canViewDemands ? api('/api/demands?page_size=100') : Promise.resolve({ data: { items: [] } }),
    canConfigure ? api('/api/poc/settings') : Promise.resolve({ data: {} }),
    api('/api/tapd/overview')
  ]);
  const demands = (demandResp.data.items || []).filter((d)=>d.demand_no);
  const overview = overviewResp.data, cfg = overview.config;
  const settings = canConfigure ? settingResp.data : {
    tapd_split_strategy: overview.split_strategy,
  ...
  const rows = demands.map((d)=>{
    ...
    return `<tr><td>${esc(d.demand_no)}</td><td><div class="row-title">${esc(d.title)}</div><small>${systems.length?`已拆分：${esc([...new Set(systems)].join('、'))}`:'尚未创建TAPD'}</small></td><td>${statusPill(d.status)}</td><td>${tapdCell}</td><td>${statusPill(syncState)}</td><td>${esc(d.tapd_last_sync_at||retry?.next_retry_at||'—')}</td><td><div class="action-group">${reqs.length ? `<button class="link tapd-sync" data-id="${d.id}">${cfg.mode==='live'?'从TAPD回读':'手动回读'}</button>${cfg.mode==='live'?`<button class="link tapd-update" data-id="${d.id}">TRM推送更新</button>`:''}` : `<button class="link tapd-push" data-id="${d.id}" ${['审批通过','TAPD同步失败'].includes(d.status) ? '' : 'disabled'}>${cfg.mode==='live'?'创建到TAPD':'创建TAPD'}</button>`}${cfg.mode==='mock' && hasPermission('tapd') && !reqs.length && ['审批通过','TAPD同步失败'].includes(d.status) ? `<button class="link tapd-fail" data-id="${d.id}">模拟失败</button>` : ''}<button class="link tapd-detail" data-id="${d.id}">完整详情</button></div></td></tr>`;
  }).join('');
  ...
  const bindRows=()=>{
    ...
    $$('.tapd-update').forEach((b)=>b.addEventListener('click',async()=>{try{const r=await api(`/api/demands/${b.dataset.id}/tapd/push-update`,{method:'POST'});toast(r.message,'success');renderTapd();}catch(error){toast(error.message,'error');}}));
  ...
  bindRows();
  if ($('#tapdSearch')) $('#tapdSearch').addEventListener('input',()=>{const q=$('#tapdSearch').value.trim().toLowerCase();$$('#tapdRows tr').forEach(tr=>{tr.style.display=!q||tr.textContent.toLowerCase().includes(q)?'':'none';});});
...
async function renderInitiativeForm(){
  ...
  const canLinkBudget=hasPermission('budget');
  const budgets=canLinkBudget?(await api('/api/budget-ledger')).data:[];
  const budgetField=canLinkBudget?`<div><label>预算来源</label><select id="iniBudget" class="select"><option value="">请选择</option>${budgets.map(b=>`<option value="${b.id}" ${b.id===item?.budget_id?'selected':''}>${esc(b.budget_no)} · ${esc(b.budget_name)}</option>`).join('')}</select></div>`:'';
  ...
  appView.innerHTML=`<div class="section"><div class="section-title">立项基本信息</div>
    ...
    <div class="grid-3" style="margin-top:10px"><div><label>项目发起人/Sponsor</label><input id="iniSponsor" class="field" value="${esc(item?.sponsor||'')}"></div><div><label>预估预算</label><input id="iniAmount" type="number" min="0" class="field" value="${item?.estimated_budget||0}"></div>${budgetField}</div>
...
async function renderInitiativeList(){
  setPage({title:'立项列表',iconName:'send',crumbs:['立项管理'],actions:canAccessRoute('initiative-form')?btn(`${icon('plus')} 新建立项`,'btn primary','iniNew'):''});
  ...
  const draw=()=>{const q=$('#iniSearch').value.trim().toLowerCase(),st=$('#iniStatus').value;const rows=items.filter(x=>(!q||`${x.initiative_no} ${x.title} ${x.applicant}`.toLowerCase().includes(q))&&(!st||x.status===st));$('#iniRows').innerHTML=rows.map(x=>`<tr><td>${x.initiative_no}</td><td>${esc(x.title)}</td><td>${esc(x.project_type||'系统建设')}</td><td>${esc(x.applicant)}</td><td>¥${money(x.estimated_budget)}</td><td>${statusPill(x.status)}</td><td>${esc(x.current_node)}</td><td><button class="link iniView" data-id="${x.id}">详情</button>${canAccessRoute('initiative-form')&&['草稿','已驳回'].includes(x.status)?` <button class="link iniEdit" data-id="${x.id}">编辑</button>`:''}</td></tr>`).join('')||'<tr><td colspan="8" class="subtle">暂无匹配数据</td></tr>';$$('.iniView').forEach(b=>b.addEventListener('click',()=>navigate(`initiative-detail/${b.dataset.id}`)));$$('.iniEdit').forEach(b=>b.addEventListener('click',()=>navigate('initiative-form',{id:b.dataset.id})));};
...
async function renderInitiativeDetail(id){
  ...
  const actions=[];if(['草稿','已驳回'].includes(x.status)&&hasPermission('initiative.create'))actions.push(btn('编辑','btn','iniEditDetail'));if(x.project_id&&hasPermission('project'))actions.push(btn('查看项目','btn primary','iniProject'));
...
async function openProjectForm(sourceItem={},pendingRelations={}){
  let item=sourceItem;
  if(item.id&&!item.contracts)item=(await api(`/api/projects/${item.id}`)).data;
  const canLinkBudget=hasPermission('budget'),canManageContracts=hasPermission('contract'),canManageSettlements=hasPermission('settlement');
  const [budgets,allContracts,allSettlements]=await Promise.all([
    canLinkBudget?api('/api/budget-ledger').then(r=>r.data):Promise.resolve([]),
    item.id&&canManageContracts?api('/api/contracts').then(r=>r.data):Promise.resolve([]),
    item.id&&canManageSettlements?api('/api/settlements').then(r=>r.data):Promise.resolve([])
  ]);
  const eligibleContracts=allContracts.filter(x=>!x.project_id||x.project_id===item.id),eligibleSettlements=allSettlements.filter(x=>!x.project_id||x.project_id===item.id);
  const currentContractIds=new Set(pendingRelations.contractIds||(item.contracts||[]).map(x=>x.id)),currentSettlementIds=new Set(pendingRelations.settlementIds||(item.settlements||[]).map(x=>x.id));
  const budgetField=canLinkBudget?`<div><label>关联预算</label><select id="pBudget" class="select"><option value="">不关联</option>${budgets.map(b=>`<option value="${b.id}" ${b.id===item.budget_id?'selected':''}>${esc(b.budget_no)} · ${esc(b.budget_name)}</option>`).join('')}</select></div>`:'';
  const relationSection=item.id?`<div class="project-relation-workbench">
    ${canManageContracts?`<div class="project-relation-column"><div class="project-relation-head"><div><b>关联合同</b><small>勾选已有未关联合同，保存后同步合同台账</small></div>${btn(`${icon('plus')} 新建合同`,'btn small','pNewContractToggle')}</div><div class="project-relation-list">${eligibleContracts.map(x=>`<label class="project-relation-option"><input type="checkbox" class="pContractLink" value="${x.id}" ${currentContractIds.has(x.id)?'checked':''}><span><b>${esc(x.contract_no)} · ${esc(x.name)}</b><small>${esc(x.supplier)} · ¥${money(x.total_amount)} · ${esc(x.status)}</small></span></label>`).join('')||'<div class="project-relation-empty">暂无可关联合同</div>'}</div>
      <div id="pNewContractPanel" class="project-inline-create hidden"><div class="grid-2"><input id="pContractName" class="field" placeholder="合同名称"><input id="pContractSupplier" class="field" placeholder="供应商"></div><div class="grid-3"><input id="pContractAmount" type="number" min="0" class="field" placeholder="合同金额"><input id="pContractStart" type="date" class="field"><input id="pContractEnd" type="date" class="field"></div><div class="project-inline-actions"><input id="pContractOwner" class="field" placeholder="合同负责人">${btn('创建并关联','btn primary small','pCreateContract')}</div></div></div>`:''}
    ${canManageSettlements?`<div class="project-relation-column"><div class="project-relation-head"><div><b>关联结算</b><small>勾选已有未关联结算，保存后同步结算台账</small></div>${btn(`${icon('plus')} 新建结算`,'btn small','pNewSettlementToggle')}</div><div class="project-relation-list">${eligibleSettlements.map(x=>`<label class="project-relation-option"><input type="checkbox" class="pSettlementLink" value="${x.id}" ${currentSettlementIds.has(x.id)?'checked':''}><span><b>${esc(x.settlement_no)} · ${esc(x.settlement_type)}</b><small>${esc(x.applicant)} · ¥${money(x.amount)} · ${esc(x.status)}</small></span></label>`).join('')||'<div class="project-relation-empty">暂无可关联结算</div>'}</div>
      <div id="pNewSettlementPanel" class="project-inline-create hidden"><div class="grid-2"><select id="pSettlementType" class="select">${['项目结算','合同付款结算','费用报销结算'].map(v=>`<option>${v}</option>`).join('')}</select><input id="pSettlementAmount" type="number" min="0" class="field" placeholder="结算金额"></div><div class="grid-2"><select id="pSettlementContract" class="select"><option value="">不关联合同</option>${(item.contracts||[]).map(x=>`<option value="${x.id}">${esc(x.contract_no)} · ${esc(x.name)}</option>`).join('')}</select><input id="pSettlementApplicant" class="field" value="${esc(state.userDisplay)}" placeholder="申请人"></div><div class="project-inline-actions"><input id="pSettlementDesc" class="field" placeholder="结算说明">${btn('创建并关联','btn primary small','pCreateSettlement')}</div></div></div>`:''}
  </div>`:'';
  showModal(`<h3 id="modalTitle">${item.id?'编辑项目':'录入存量项目'}</h3>${item.id?'':`<div class="callout project-source-callout"><b>存量项目录入</b><span>仅用于已完成线下立项审批或历史数据迁移；常规项目请从“发起立项”进入审批流程。</span></div>`}<div class="form-row"><div class="label required">项目名称</div><input id="pName" class="field" value="${esc(item.name||'')}"></div><div class="grid-2"><div><label>项目经理</label><input id="pManager" class="field" value="${esc(item.manager||'王卫嘉')}"></div><div><label>部门</label><input id="pDept" class="field" value="${esc(item.department||'数字化管理部')}"></div></div><div class="grid-2" style="margin-top:10px">${budgetField}<div><label>项目预算</label><input id="pTotal" type="number" min="0" class="field" value="${item.total_budget||0}"></div></div><div class="grid-3" style="margin-top:10px"><div><label>状态</label><select id="pStatus" class="select">${['规划中','实施中','暂停','已完成'].map(s=>`<option ${s===item.status?'selected':''}>${s}</option>`).join('')}</select></div><div><label>进度</label><input id="pProgress" type="number" min="0" max="100" class="field" value="${item.progress||0}"></div><div><label>开始日期</label><input id="pStart" type="date" class="field" value="${item.start_date||''}"></div></div><div style="margin-top:10px"><label>结束日期</label><input id="pEnd" type="date" class="field" value="${item.end_date||''}"></div><div style="margin-top:10px"><label>项目说明</label><textarea id="pDesc" class="textarea">${esc(item.description||'')}</textarea></div>${relationSection}<div class="modal-actions">${btn('取消','btn','pCancel')}${btn(item.id?'保存项目与关联':'确认录入','btn primary','pSave')}</div>`,true);
  $('#pCancel').addEventListener('click',closeModal);
  const currentProjectDraft=()=>({name:$('#pName').value,manager:$('#pManager').value,department:$('#pDept').value,budget_id:Number($('#pBudget')?.value)||null,total_budget:Number($('#pTotal').value),status:$('#pStatus').value,progress:Number($('#pProgress').value),start_date:$('#pStart').value||null,end_date:$('#pEnd').value||null,description:$('#pDesc').value});
  const pendingRelationDraft=()=>({contractIds:canManageContracts?$$('.pContractLink:checked').map(x=>Number(x.value)):[],settlementIds:canManageSettlements?$$('.pSettlementLink:checked').map(x=>Number(x.value)):[]});
  if($('#pNewContractToggle'))$('#pNewContractToggle').addEventListener('click',()=>$('#pNewContractPanel').classList.toggle('hidden'));
  if($('#pNewSettlementToggle'))$('#pNewSettlementToggle').addEventListener('click',()=>$('#pNewSettlementPanel').classList.toggle('hidden'));
  if($('#pCreateContract'))$('#pCreateContract').addEventListener('click',async()=>{try{const name=$('#pContractName').value.trim(),supplier=$('#pContractSupplier').value.trim(),amount=Number($('#pContractAmount').value);if(!name||!supplier||amount<=0)return toast('请完整填写合同名称、供应商和合同金额','error');const draft=currentProjectDraft(),pending=pendingRelationDraft();const created=await api('/api/contracts',{method:'POST',body:JSON.stringify({name,project_id:item.id,budget_id:draft.budget_id||item.budget_id||null,supplier,total_amount:amount,start_date:$('#pContractStart').value||null,end_date:$('#pContractEnd').value||null,owner:$('#pContractOwner').value,description:`由项目 ${item.project_no} 创建`})});pending.contractIds.push(created.data.id);const fresh=(await api(`/api/projects/${item.id}`)).data;toast('合同已创建并同步到合同台账','success');await openProjectForm({...fresh,...draft,contracts:fresh.contracts,settlements:fresh.settlements},pending);}catch(e){toast(e.message,'error')}});
  if($('#pCreateSettlement'))$('#pCreateSettlement').addEventListener('click',async()=>{try{const amount=Number($('#pSettlementAmount').value);if(amount<=0)return toast('结算金额必须大于0','error');const draft=currentProjectDraft(),pending=pendingRelationDraft();const created=await api('/api/settlements',{method:'POST',body:JSON.stringify({project_id:item.id,contract_id:Number($('#pSettlementContract').value)||null,budget_id:draft.budget_id||item.budget_id||null,amount,settlement_type:$('#pSettlementType').value,applicant:$('#pSettlementApplicant').value,description:$('#pSettlementDesc').value||`由项目 ${item.project_no} 创建`})});pending.settlementIds.push(created.data.id);const fresh=(await api(`/api/projects/${item.id}`)).data;toast('结算单已创建并同步到结算台账','success');await openProjectForm({...fresh,...draft,contracts:fresh.contracts,settlements:fresh.settlements},pending);}catch(e){toast(e.message,'error')}});
  $('#pSave').addEventListener('click',async()=>{try{const name=$('#pName').value.trim(),manager=$('#pManager').value.trim();if(!name||!manager)return toast('请填写项目名称和项目经理','error');const p={name,manager,department:$('#pDept').value,budget_id:Number($('#pBudget')?.value)||null,total_budget:Number($('#pTotal').value),status:$('#pStatus').value,progress:Number($('#pProgress').value),start_date:$('#pStart').value||null,end_date:$('#pEnd').value||null,description:$('#pDesc').value};const r=await api(item.id?`/api/projects/${item.id}`:'/api/projects',{method:item.id?'PUT':'POST',body:JSON.stringify(p)});const projectId=item.id||r.data.id;if(item.id&&(canManageContracts||canManageSettlements)){await api(`/api/projects/${projectId}/relations`,{method:'PUT',body:JSON.stringify({contract_ids:canManageContracts?$$('.pContractLink:checked').map(x=>Number(x.value)):null,settlement_ids:canManageSettlements?$$('.pSettlementLink:checked').map(x=>Number(x.value)):null})});}closeModal();toast(item.id?'项目与关联信息已同步':r.message,'success');item.id?renderProjectDetail(projectId):renderProjectList();}catch(e){toast(e.message,'error')}});
}
async function renderProjectDetail(id,query){
  ...
  $$('.pSettlementView').forEach(b=>b.addEventListener('click',()=>navigate(`settlement-detail/${b.dataset.id}`)));
  $('#open360').addEventListener('click',()=>navigate('project360',{projectId:id}));
}
...
async function renderSettlementForm(){
  const id=Number(parseRoute().query.get('id')||0);
  const canLinkProject=hasPermission('project360')||hasPermission('project');
  const canLinkContract=hasPermission('contract');
  const canLinkBudget=hasPermission('budget');
  const [projects,contracts,budgets,all]=await Promise.all([
    canLinkProject?api('/api/projects').then(r=>r.data):Promise.resolve([]),
    canLinkContract?api('/api/contracts').then(r=>r.data):Promise.resolve([]),
    canLinkBudget?api('/api/budget-ledger').then(r=>r.data):Promise.resolve([]),
    api('/api/settlements').then(r=>r.data)
  ]);
  const item=id?all.find(x=>x.id===id):null;
  const projectField=canLinkProject?`<div><label>项目</label><select id="setProject" class="select"><option value="">请选择</option>${projects.map(x=>`<option value="${x.id}" ${x.id===item?.project_id?'selected':''}>${x.project_no} · ${esc(x.name)}</option>`).join('')}</select></div>`:'';
  const contractField=canLinkContract?`<div><label>合同</label><select id="setContract" class="select"><option value="">请选择</option>${contracts.map(x=>`<option value="${x.id}" ${x.id===item?.contract_id?'selected':''}>${x.contract_no} · ${esc(x.name)}</option>`).join('')}</select></div>`:'';
  const budgetField=canLinkBudget?`<div><label>预算</label><select id="setBudget" class="select"><option value="">请选择</option>${budgets.map(x=>`<option value="${x.id}" ${x.id===item?.budget_id?'selected':''}>${x.budget_no} · ${esc(x.budget_name)}</option>`).join('')}</select></div>`:'';
  setPage({title:'结算申请',iconName:'settle',crumbs:['结算管理'],actions:`${btn('保存草稿','btn','setSave')}${btn('提交审批','btn primary','setSubmit')}`});
  appView.innerHTML=`<div class="section"><div class="section-title">结算基本信息</div><div class="grid-3"><div><label>结算类型</label><select id="setType" class="select">${['项目结算','合同付款结算','费用报销结算'].map(v=>`<option ${v===item?.settlement_type?'selected':''}>${v}</option>`).join('')}</select></div>${projectField}${contractField}</div><div class="grid-3" style="margin-top:10px">${budgetField}<div><label>结算金额</label><input id="setAmount" type="number" class="field" value="${item?.amount||0}"></div><div><label>申请人</label><input id="setApplicant" class="field" value="${esc(item?.applicant||state.userDisplay)}"></div></div><div style="margin-top:10px"><label>结算说明</label><textarea id="setDesc" class="textarea">${esc(item?.description||'')}</textarea></div>${item?`<div class="callout" style="margin-top:12px">${item.settlement_no} · ${statusPill(item.status)} · ${esc(item.current_node)}<div class="help">保存后可在结算详情中维护明细项，明细合计会自动回写结算金额。</div></div>`:''}</div>`;
...
function openAccountProfile() {
  ...
  showModal(`<h3 id="modalTitle">账号信息</h3>
    <div class="profile-card"><div class="profile-avatar">${esc((u.display_name||u.username||'U').slice(0,1))}</div><div><strong>${esc(u.display_name||'—')}</strong><p>${esc(u.username||'—')} · ${esc(currentRoleLabels())}</p></div></div>
...
async function renderUsers(){
  ...
  setPage({title:'用户管理',iconName:'user',crumbs:['系统管理'],actions:canManageRoles?btn(`${icon('plus')} 新增用户`,'btn primary','userNew'):''});
  const [usersResp,rolesResp]=await Promise.all([api('/api/system/users'),canManageRoles?api('/api/system/roles'):Promise.resolve({data:[]})]);
...
function openUserForm(item,roles){
  ...
  const selected=new Set(item?.role_codes||[item?.role_code].filter(Boolean));
  showModal(`<h3 id="modalTitle">${isEdit?'编辑用户':'新增用户'}</h3><div class="grid-2"><div><label>登录账号 *</label><input id="uUsername" class="field" value="${esc(item?.username||'')}" ${item?.username==='admin'?'disabled':''}></div><div><label>姓名 *</label><input id="uDisplay" class="field" value="${esc(item?.display_name||'')}"></div></div><div class="grid-2" style="margin-top:10px"><div><label>所属部门</label><input id="uDept" class="field" value="${esc(item?.department||'')}"></div><div><label>账号状态</label><select id="uStatus" class="select"><option ${item?.status!=='停用'?'selected':''}>启用</option><option ${item?.status==='停用'?'selected':''}>停用</option></select></div></div><div style="margin-top:12px"><label>用户角色 *（可多选，权限取并集）</label><div class="user-role-grid">${roles.map(r=>`<label><input type="checkbox" class="userRoleCheck" value="${esc(r.code)}" ${selected.has(r.code)?'checked':''}><span>${esc(r.label)}</span><small>${esc(r.code)}</small></label>`).join('')}</div></div><div class="grid-2" style="margin-top:10px"><div><label>邮箱</label><input id="uEmail" class="field" value="${esc(item?.email||'')}"></div><div><label>手机</label><input id="uPhone" class="field" value="${esc(item?.phone||'')}"></div></div><div style="margin-top:10px"><label>${isEdit?'新密码（留空不修改）':'初始密码'}</label><input id="uPassword" class="field" type="password" placeholder="${isEdit?'留空保持原密码':'默认 Demo@123'}"></div><div class="help">保存后即时生效：页面、后端 API 与 AI/MCP 共用该用户所有启用角色的权限并集。</div><div class="modal-actions">${btn('取消','btn','uCancel')}${btn('保存','btn primary','uSave')}</div>`,true);
  $('#uCancel').addEventListener('click',closeModal);
  $('#uSave').addEventListener('click',async()=>{
    const roleCodes=$$('.userRoleCheck:checked').map(c=>c.value);
    const payload={username:(item?.username==='admin'?'admin':$('#uUsername').value.trim()),display_name:$('#uDisplay').value.trim(),department:$('#uDept').value,email:$('#uEmail').value,phone:$('#uPhone').value,role_codes:roleCodes,status:$('#uStatus').value,password:$('#uPassword').value||null};
    if(!payload.username||!payload.display_name)return toast('请填写账号和姓名','error');
    if(!roleCodes.length)return toast('请至少选择一个角色','error');
    try{const r=await api(isEdit?`/api/system/users/${item.id}`:'/api/system/users',{method:isEdit?'PUT':'POST',body:JSON.stringify(payload)});closeModal();toast(r.message,'success');renderUsers();}catch(e){toast(e.message,'error')}
  });
}
...
function openRoleForm(item,permissions){
  ...
  $('#rAll').addEventListener('change',e=>$$('.permCheck').forEach(c=>{c.disabled=e.target.checked;c.checked=e.target.checked;}));$('#rCancel').addEventListener('click',closeModal);$('#rSave').addEventListener('click',async()=>{const perms=$('#rAll').checked?['*']:$$('.permCheck:checked').map(c=>c.value);const payload={code:edit?item.code:$('#rCode').value.trim(),label:$('#rLabel').value.trim(),description:$('#rDesc').value,permissions:perms,status:$('#rStatus').value};if(!payload.code||!payload.label)return toast('请填写角色编码和名称','error');try{const r=await api(edit?`/api/system/roles/${encodeURIComponent(item.code)}`:'/api/system/roles',{method:edit?'PUT':'POST',body:JSON.stringify(payload)});closeModal();toast(r.message,'success');if((state.currentUserData?.role_codes||[]).includes(item?.code)){state.currentUserData=(await api('/api/auth/me')).data;state.role=state.currentUserData.role_code;updateUserUI();}renderRoles();}catch(e){toast(e.message,'error')}});
}

async function renderIntegrations(){
  setPage({title:'集成配置',iconName:'settings',crumbs:['系统管理']});
  const items=(await api('/api/integrations')).data;
  appView.innerHTML=`<div class="grid-3">${items.map(x=>`<div class="metric"><div class="toolbar"><div><div class="k">${esc(x.code.toUpperCase())}</div><div class="v" style="font-size:17px">${esc(x.name)}</div></div>${statusPill(x.enabled?x.status:'停用')}</div><div class="sub">模式：${esc(x.mode)} · ${x.base_url?esc(x.base_url):'内置服务'}</div>${x.code==='ai'?`<div class="sub" style="margin-top:6px">智能体：<span class="detail-no">${esc(x.agent_id||'未配置')}</span></div>`:''}${x.code==='mcp'?'<div class="sub" style="margin-top:6px">协议：Streamable HTTP · Bearer鉴权 · 13个工具</div>':''}<div class="sub" style="margin-top:6px">${esc(x.description)}</div><div class="action-group" style="margin-top:12px"><button class="btn intCheck" data-code="${x.code}">连通性检查</button>${hasPermission('system.integrations')?`<button class="btn primary intEdit" data-code="${x.code}">配置</button>`:''}</div></div>`).join('')}</div>`;
