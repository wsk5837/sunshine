...
function currentUser() {
  return state.meta?.demoUsers?.[state.role] || { id: state.actorId, name: state.userDisplay, dept: '' };
}
...
function navigate(path, query = {}) {
  const group = routeGroup(String(path).split('/')[0]);
...
function updateUserUI() {
  ...
  state.userDisplay = user.name;
  $('#userName').textContent = user.name;
  $('#userRoleLabel').textContent = state.meta.roles[state.role];
  $('#userAvatar').textContent = user.name.trim().slice(0, 1);
  $('#roleOptions').innerHTML = Object.entries(state.meta.roles).map(([role, label]) => {
    const info = state.meta.demoUsers[role];
    return `<button class="role-option ${role === state.role ? 'active' : ''}" type="button" data-role="${role}"><strong>${esc(label)}</strong><small>${esc(info.name)}</small></button>`;
  }).join('');
  $$('.role-option').forEach((button) => button.addEventListener('click', async () => {
    state.role = button.dataset.role;
    updateUserUI();
    $('#userMenu').classList.add('hidden');
    $('#userTrigger').setAttribute('aria-expanded', 'false');
    toast(`已切换身份：${state.meta.roles[state.role]}`, 'success');
    await loadNotifications(false);
    renderRoute();
  }));
}
...
function routeGroup(base) {
  const groups = {
    ...
    system: new Set(['integrations','audit'])
  };
...
function bindShell() {
  $('#sideNav').addEventListener('click', (event) => {
    ...
    if (groupToggle) {
      ...
      if (state.openGroups.has(name)) state.openGroups.delete(name); else state.openGroups.add(name);
  ...
  $('#logoutBtn').addEventListener('click', () => {
    showModal(`<h3 id="modalTitle">退出登录</h3><div class="callout warn">当前部署使用本地演示账号体系；接入企业SSO后可替换为统一认证会话。</div><p>可以将当前演示身份重置为需求申请人并返回首页。</p><div class="modal-actions">${btn('取消', 'btn', 'cancelLogout')}${btn('重置并返回首页', 'btn primary', 'confirmLogout')}</div>`);
...
async function init() {
  ...
  if (!location.hash) {
    ...
    const initialGroup = routeGroup(parseRoute().path.split('/')[0]);
    if (initialGroup) state.openGroups.add(initialGroup);
    await renderRoute();
  }
}
...
async function renderDashboard() {
  setPage({ title: '首页', iconName: 'home', crumbs: ['驾驶舱'], actions: `${btn(`${icon('plus')} 新建需求`, 'btn primary', 'dashNew')}${btn('导出汇总','btn','dashExport')}` });
  const p = (await api('/api/platform-dashboard')).data;
  ...
  appView.innerHTML = `<div class="grid-4">
    ...
    <div class="metric"><div class="k">需求总数</div><div class="kpi-number">${p.demands}</div><div class="sub">端到端需求全生命周期</div></div>
    ...
    <div class="metric"><div class="k">跨模块待审批</div><div class="kpi-number">${p.pending_approvals}</div><div class="sub">立项 / 合同 / 结算</div></div>
  </div>
  ...
  <div class="grid-2" style="margin-top:12px"><div class="section"><div class="section-title">快捷入口</div><div class="quick-grid">${btn('新建立项','btn','quickIni')}${btn('新建需求','btn primary','quickDemand')}${btn('项目台账','btn','quickProject')}${btn('合同台账','btn','quickContract')}${btn('预算管理','btn','quickBudget')}${btn('结算管理','btn','quickSettlement')}</div></div><div class="section"><div class="section-title">最近立项</div>${simpleTable(['立项编号','名称','状态','当前节点'],p.recent_initiatives.map(x=>[x.initiative_no,x.title,statusPill(x.status),x.current_node]))}</div></div>`;
  bindCommonDemandActions(appView);
  $('#dashNew').addEventListener('click',()=>navigate('demand-form'));
  if ($('#dashExport')) $('#dashExport').addEventListener('click',()=>window.open('/api/exports/platform-summary.csv','_blank')); $('#quickDemand').addEventListener('click',()=>navigate('demand-form')); $('#quickIni').addEventListener('click',()=>navigate('initiative-form')); $('#quickProject').addEventListener('click',()=>navigate('project-list')); $('#quickContract').addEventListener('click',()=>navigate('contract-list')); $('#quickBudget').addEventListener('click',()=>navigate('budget')); $('#quickSettlement').addEventListener('click',()=>navigate('settlement-list'));
}

function statusPill(s){return `<span class="status ${statusClass(s)}">${esc(s)}</span>`;}
function progressCell(v){return `<div style="min-width:110px"><div class="progress"><span style="width:${Math.max(0,Math.min(100,Number(v||0)))}%"></span></div><small>${Number(v||0)}%</small></div>`;}
...
async function renderDemandForm() {
  ...
  if (idFromQuery) state.currentDemandId = idFromQuery;
  let demand = state.currentDemandId ? (await api(`/api/demands/${state.currentDemandId}`)).data : null;
  ...
  setPage({ title: '需求申请', iconName: 'demand', crumbs: ['需求申请'], actions });
...
async function renderDemandList() {
  setPage({ title: '需求查询', iconName: 'search', crumbs: ['需求管理'], actions: `${btn('导出当前结果','btn','exportDemandList')}${btn(`${icon('plus')} 新建需求`, 'btn primary', 'newDemandFromList')}` });
  const route = parseRoute();
  ...
  bindCommonDemandActions(appView);
  $('#newDemandFromList').addEventListener('click', () => { state.currentDemandId = null; navigate('demand-form'); });
...
function exportDemandCsv(items) {
  ...
  const a = document.createElement('a'); a.href = url; a.download = `需求查询_${new Date().toISOString().slice(0,10)}.csv`; a.click(); URL.revokeObjectURL(url);
}
...
async function renderTapd() {
  ...
  const [demandResp, settingResp] = await Promise.all([api('/api/demands?page_size=100'), api('/api/poc/settings')]);
  const demands = demandResp.data.items.filter((d)=>d.demand_no);
  const settings = settingResp.data;
  ...
  const rows = demands.map((d)=>{
    const reqs = d.tapd_requirements || [];
    const retry = d.tapd_retry_job;
    const systems = reqs.map((r)=>r.system_name).filter(Boolean);
    const tapdCell = reqs.length ? reqs.map((r)=>`<div><strong>${esc(r.tapd_id)}</strong><small>${esc(r.system_name||'默认系统')} · ${esc(r.tapd_status||'新')}</small></div>`).join('') : '—';
    const syncState = retry && retry.status==='等待重试' ? `等待第${Number(retry.attempt_count||1)+1}次重试` : (d.tapd_sync_status||'未同步');
    return `<tr><td>${esc(d.demand_no)}</td><td><div class="row-title">${esc(d.title)}</div><small>${systems.length?`已拆分：${esc([...new Set(systems)].join('、'))}`:'尚未创建TAPD'}</small></td><td>${statusPill(d.status)}</td><td>${tapdCell}</td><td>${statusPill(syncState)}</td><td>${esc(d.tapd_last_sync_at||retry?.next_retry_at||'—')}</td><td><div class="action-group">${reqs.length ? `<button class="link tapd-sync" data-id="${d.id}">手动回读</button>` : `<button class="link tapd-push" data-id="${d.id}" ${['审批通过','TAPD同步失败'].includes(d.status) ? '' : 'disabled'}>创建TAPD</button>`}${['project_manager','admin'].includes(state.role) && !reqs.length && ['审批通过','TAPD同步失败'].includes(d.status) ? `<button class="link tapd-fail" data-id="${d.id}">模拟失败</button>` : ''}<button class="link tapd-detail" data-id="${d.id}">完整详情</button></div></td></tr>`;
  }).join('');
  appView.innerHTML = `${workflow(4)}
  <div class="grid-3">
    <div class="metric"><div class="k">定时回读</div><div class="v" style="font-size:17px">每${intervalMin}分钟</div><div class="sub">后台任务真实执行；单批≤2000条</div></div>
    <div class="metric"><div class="k">Webhook</div><div class="v" style="font-size:17px">已启用</div><div class="sub">POST /api/tapd/webhook · 状态变更主动回读</div></div>
    <div class="metric"><div class="k">异常重试</div><div class="v" style="font-size:17px">3次 × ${settings.tapd_retry_seconds||30}秒</div><div class="sub">持久化重试队列 + REQ幂等 + 最终失败告警</div></div>
  </div>
  <div class="section" style="margin-top:12px">
    <div class="toolbar"><div><div class="section-title">TAPD创建策略</div><div class="section-subtitle">需求涉及多个系统时，当前按“${strategyLabel}”生成TAPD需求。</div></div>${state.role==='admin'?`<div class="action-group"><select id="tapdStrategy" class="select" style="min-width:190px"><option value="system" ${settings.tapd_split_strategy==='system'?'selected':''}>按系统生成</option><option value="system_allocation" ${settings.tapd_split_strategy==='system_allocation'?'selected':''}>按系统 + 分摊行生成</option></select>${btn('保存策略','btn primary','saveTapdStrategy')}</div>`:''}</div>
    <div class="callout">字段映射包含：REQ外部ID、标题、描述、需求类型、预算出处、优先级（高→High / 中→Medium / 低→Low）、申请人及附件；成功后记录TAPD ID、URL、创建时间和同步状态。</div>
  </div>
  <div class="section" style="margin-top:12px"><div class="section-title">TAPD需求创建与信息回读</div><div class="table-wrap"><table class="table"><thead><tr><th>需求编号</th><th>需求标题 / 拆分系统</th><th>系统状态</th><th>TAPD需求</th><th>同步 / 重试状态</th><th>最近同步 / 下次重试</th><th>操作</th></tr></thead><tbody>${rows||'<tr><td colspan="7"><div class="empty">暂无可同步需求</div></td></tr>'}</tbody></table></div></div>`;
  $('#tapdRefresh').addEventListener('click', renderTapd);
  if ($('#tapdRunJobs')) $('#tapdRunJobs').addEventListener('click', async()=>{try{const r=await api('/api/poc/jobs/scan?force=true',{method:'POST'});toast(`后台任务执行完成：OA提醒${r.data.oaReminders}，TAPD重试${r.data.tapdRetries}，定时回读${r.data.tapdScheduledSync}`,'success');await loadNotifications(false);renderTapd();}catch(e){toast(e.message,'error')}});
  if ($('#saveTapdStrategy')) $('#saveTapdStrategy').addEventListener('click', async()=>{try{await api('/api/poc/settings',{method:'PUT',body:JSON.stringify({tapd_split_strategy:$('#tapdStrategy').value})});toast('TAPD拆分策略已保存','success');renderTapd();}catch(e){toast(e.message,'error')}});
  $$('.tapd-detail').forEach((b)=>b.addEventListener('click',()=>navigate(`demand-detail/${b.dataset.id}`,{tab:'tapd'})));
  $$('.tapd-sync').forEach((b)=>b.addEventListener('click',async()=>{const d=(await api(`/api/demands/${b.dataset.id}`)).data;openTapdSyncModal(d);}));
...
function openTapdSyncModal(demand) {
  showModal(`<h3 id="modalTitle">同步TAPD状态</h3><div class="callout">${esc(demand.demand_no)} · ${esc(demand.title)}<div class="help">当前TAPD状态：${esc(demand.tapd_status || '新')}</div></div><div class="form-row" style="margin-top:14px"><div class="label">回读状态</div><select class="select" id="tapdSyncStatus">${['新','开发中','测试中','已验收','已关闭','已拒绝'].map((s)=>`<option ${s===demand.tapd_status?'selected':''}>${s}</option>`).join('')}</select></div><div class="modal-actions">${btn('取消','btn','cancelTapdSync')}${btn('立即同步','btn primary','confirmTapdSync')}</div>`);
  $('#cancelTapdSync').addEventListener('click',closeModal);
  $('#confirmTapdSync').addEventListener('click',async()=>{try{const status=$('#tapdSyncStatus').value;const r=await api(`/api/demands/${demand.id}/tapd/sync?tapd_status=${encodeURIComponent(status)}`,{method:'POST'});toast(r.message,'success');closeModal();await loadNotifications(false);renderRoute();}catch(error){toast(error.message,'error');}});
}
...
async function renderInitiativeForm(){
  ...
  setPage({title:'立项申请',iconName:'send',crumbs:['立项申请'],actions:`${btn('保存草稿','btn','iniSave')}${item?btn('删除草稿','btn danger','iniDelete'):''}${btn('提交审批','btn primary','iniSubmit',item&& !['草稿','已驳回'].includes(item.status)?'disabled':'')}`});
...
async function renderInitiativeList(){
  setPage({title:'立项查询',iconName:'send',crumbs:['立项申请'],actions:btn(`${icon('plus')} 新建立项`,'btn primary','iniNew')});
  ...
  $('#iniSearch').addEventListener('input',draw);$('#iniStatus').addEventListener('change',draw);$('#iniNew').addEventListener('click',()=>navigate('initiative-form'));draw();
}
async function renderInitiativeDetail(id){
  ...
  setPage({title:'立项详情',iconName:'send',crumbs:['立项申请','立项查询'],actions:actions.join('')});
...
async function renderInitiativeApprovals(){setPage({title:'立项审批',iconName:'approve',crumbs:['立项申请']});const items=(await api('/api/initiative-approvals/pending')).data;appView.innerHTML=`<div class="section"><div class="section-title">我的立项审批待办</div>${simpleTable(['立项编号','名称','预算','当前节点','操作'],items.map(x=>[x.initiative_no,esc(x.title),`¥${money(x.estimated_budget)}`,x.current_node,`<button class="link iniApprove" data-id="${x.id}">处理</button>`]))}</div>`;$$('.iniApprove').forEach(b=>b.addEventListener('click',async()=>{const x=(await api(`/api/initiatives/${b.dataset.id}`)).data;showModal(`<h3 id="modalTitle">${esc(x.current_node)}</h3><div class="callout">${esc(x.initiative_no)} · ${esc(x.title)} · ¥${money(x.estimated_budget)}</div><textarea id="iniComment" class="textarea" placeholder="审批意见"></textarea><div class="modal-actions">${btn('驳回','btn danger','iniReject')}${btn('通过','btn primary','iniPass')}</div>`);for(const [id,action] of [['iniReject','驳回'],['iniPass','通过']])$('#'+id).addEventListener('click',async()=>{await api(`/api/initiatives/${x.id}/approve`,{method:'POST',body:JSON.stringify({action,comment:$('#iniComment').value})});closeModal();toast(`已${action}`,'success');renderInitiativeApprovals();});}));}
