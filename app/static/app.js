...
async function renderProject360() {
  ...
  if(!selected){appView.innerHTML='<div class="empty">暂无项目，请先发起立项并在审批通过后生成项目。</div>';return;}
...
async function renderInitiativeDetail(id){
  ...
  const actions=[];if(['草稿','已驳回'].includes(x.status)&&hasPermission('initiative.create'))actions.push(btn('编辑','btn','iniEditDetail'));if(x.status==='已通过'&&hasPermission('project'))actions.push(btn('生成项目','btn primary','iniConvert'));
