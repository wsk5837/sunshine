# TRM 科技资源管理系统 V5.1.3（投入管理界面精简版）

本版仅在投入分析驾驶舱保留统计卡片，计划、审批、调整、执行、预警及详情弹窗改用简洁台账、页签数量或行内信息。完整保留 V5.1.2 的筛选、排序、分页、导出、逐项核定、调整重提及核销流水功能。变更和实测范围见 [V5.1.3_界面精简与验收.md](V5.1.3_界面精简与验收.md)。其他模块与现有数据库、Render 配置保留。
