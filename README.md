...
- 项目编辑内置合同/结算关联工作台，可关联已有单据或直接新建并同步对应台账；常规项目从立项审批生成，直接创建入口明确为“录入存量项目”。
...
- `trm_prepare_log_work_hours` -> `trm_log_work_hours`：预览并登记实际工时，自动汇总并重算预警。
...
- 产品经理或项目经理可在需求详情维护预估工时、计划完成日期，并逐笔登记/删除实际工时；实际工时由明细自动汇总，不再是只读展示值。
